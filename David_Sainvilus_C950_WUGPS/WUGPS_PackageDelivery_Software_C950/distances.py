...
# Read CSV files
with open('csv_data/distance_data.csv') as csv_file_1:
    distance_csv = list(csv.reader(csv_file_1, delimiter=','))

with open('csv_data/distance_name_data.csv') as csv_file_2:
    distance_name_csv = list(csv.reader(csv_file_2, delimiter=','))

    # Get package address csv_data -> O(n)
    def get_address():
    ...
    # The following algorithm is called the  'greedy approach'.
    # This is algorithm uses a recursive technique to determine the
    # best location to visit next based on the current location.

    # This algorithm contains 3 parameters:
    # 1. List of packages, 2. Truck number,  3. Current location of the truck

    # The first for loop finds the shortest distance to the next location.
    # The lowest_value will continually change until
    # a minimum value is found.

    # The second for loop determinism what happens when the lowest value has
    # been decided. Conditionally statements check which truck
